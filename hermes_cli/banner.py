...
def check_for_updates() -> Optional[int]:
    ...
    # A source checkout has no HERMES_REVISION, so VERSION alone cannot bind
    # the cache to the installed bytes: local merges/cherry-picks commonly keep
    # the package version unchanged. Include the live HEAD SHA in the cache key
    # so an update immediately invalidates a result produced by the old tree.
    if not embedded_rev:
    ...
    # Read cache — invalidate if the embedded/live revision OR installed
    # version has changed since the last check.
    now = time.time()
    try:
