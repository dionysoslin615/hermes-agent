...
def init_agent(
    ...
    # may legitimately expose kanban_show in normal chat, so tool availability
    # alone is not proof that this process owns a dispatched task. Require the
    # dispatcher's task marker as well; real workers keep the full protocol,
    # normal/orchestrator chats keep their Kanban tools without worker guidance.
    from agent.prompt_builder import KANBAN_GUIDANCE
    _is_dispatched_kanban_worker = bool(os.environ.get("HERMES_KANBAN_TASK"))
    agent._kanban_worker_guidance = (
        KANBAN_GUIDANCE
        if _is_dispatched_kanban_worker and "kanban_show" in agent.valid_tool_names
        else ""
    )
