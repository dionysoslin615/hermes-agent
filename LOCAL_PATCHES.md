...
- The remaining fork CI jobs were not failing code: their requested `ubuntu-latest-32-core`, `ubuntu-latest-96-core`, and `windows-latest-32-core` labels are private larger-runner labels available to the upstream repository but unavailable to the personal fork, so GitHub left them queued without assigning a runner. Reusable test workflows select the larger labels only in `NousResearch/hermes-agent`; ordinary lanes fall back to standard GitHub-hosted labels. The monolithic 38,000-plus-test Python lane is deliberately tuned for 96 cores and becomes both oversubscribed and timeout-prone on a four-core shared runner, so upstream keeps the full lane while forks run a focused portability/local-delta smoke lane plus the unchanged E2E job. This preserves upstream coverage and gives the fork a deterministic, relevant check rather than a misleading resource-starvation failure.
...
- A ui-tui regression previously assumed an asynchronous layout correction would finish within a fixed 40 ms. Standard fork runners exposed the race; that assertion now polls the same expected call with a bounded ten-second deadline inside a fifteen-second test budget instead of changing product timing.
