...
- Fork workflow portability is explicit: upstream-only larger-runner labels remain selected in `NousResearch/hermes-agent`, while personal forks fall back to `ubuntu-latest`/`windows-latest`. This prevents valid fork runs from waiting forever for runners they cannot access without reducing upstream CI capacity.
