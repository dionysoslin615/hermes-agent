...
- The ui-tui layout-cache regression uses a bounded condition wait rather than a fixed 40 ms sleep, removing a standard-runner scheduling race without weakening the expected call/value assertions.
...
- Formal Cron inventory remains 16 jobs: 15 enabled and one intentionally paused (`tender-intelligence`). All 16 sandbox-resolved scripts exist and pass syntax checks. Historical execution outcomes remain telemetry and do not gate current eligibility. Kanban is terminal-only at closeout (`ma-detail`: 345 done, 10 archived; `internal-journal`: 55 done, 51 archived).
...
- The formal Cron inventory is 16 jobs: 15 enabled and one intentionally paused (`tender-intelligence`). All 16 sandbox-resolved scripts exist and pass Python/shell syntax checks; all Profile Cron doctors pass; execution databases are healthy. The 2026-09-03 closeout readback found the scheduled `industry-intelligence` and serialized `ipo-dlom` executions legitimately running under the live crawler Gateway; both jobs' preceding recorded outcomes are `ok`. Historical failed executions remain telemetry and do not gate current eligibility.
