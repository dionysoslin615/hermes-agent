...
- At the owner's correction, the experimental `agent-browser` 0.36 path and all proposed new Hermes repository-source changes were discarded. A later closeout audit also found that the Lightpanda 0.3.7 downgrade and `services/browser-runtime/bin/agent-browser` wrapper were created solely to satisfy an upgrade-authored Lightpanda smoke, not an owner production requirement. Both were retired. The pre-upgrade browser contract is restored exactly: npm-native `agent-browser` 0.26.0 is the only agent-browser entry, Chrome remains the active/default path, and official Lightpanda 0.4.0 remains installed but activation-deferred under `runtime-manifest.json`. No wrapper or new browser runtime source remains.
...
### Post-upgrade deep-clean and repair closeout (2026-09-03)

- The deep inventory removed all unreferenced non-Tender backup, history, rollback, candidate, test, browser, build, socket and audit residue in two passes. A newly generated non-Tender recovery/probe backup and all validation staging/rollback material were also removed before closeout. The root backup directory contains only the five owner-retained Tender groups; Profile backup contents are empty. Git-tracked source assets and consumer-bound business evidence remain outside the cleanup class.
- MinerU cleanup was evidence-led: 52 passed jobs whose final Markdown SHA256 matched a formal KB file, their superseded/test predecessors, and one failed manual-repair job with a later passed and formally published same-input replacement were removed. MinerU temporary jobs are empty. Remaining passed jobs without a separate byte-identical formal copy, one unresolved manual-repair job, and three incomplete/no-manifest jobs remain protected; the stopped production container, image and its 394.1 MB writable cache remain by owner decision.
- Browser executable and engine-path configuration is centralized: Profile `.env` files do not duplicate those two settings, and the central `environment.d` file declares the governed engine root and Chrome executable. PAC routing is intentionally Profile-scoped across all seven formal Profiles through `AGENT_BROWSER_ARGS=--no-sandbox,--disable-dev-shm-usage,--proxy-pac-url=http://127.0.0.1:7788/proxy.pac`; the PAC itself proxies only whitelist matches and returns `DIRECT` otherwise. Tender's Firecrawl/direct-browser/PAC escalation order applies only to Tender and must not be generalized to other Cron or business tasks. A Playwright headless-shell revision alias resolves to the same Chrome 152 executable and contains no duplicate binary. Seven live doctors passed Browser and every configured MCP server; seven independent primary-model smokes returned exact `PROFILE_OK`.
- The DLOM full-database backup leak is closed outside Hermes core: each backup is retained through commit, integrity/readback, completion receipt and completed queue state, then hash-verified and removed. The receipt retains backup path, digest and deletion timestamp. The full focused suite passes 86 tests. Existing 658 snapshots were removed after the running project completed normally.
- Remote Git recovery covered both transport and CI: `gh auth setup-git` fixed unauthenticated HTTPS fetch throttling; invalid inputs passed to the local `detect-changes` composite action were removed; and three intentionally POSIX-only local-patch references were made explicit to the blocking Windows-footgun checker without changing their guarded behavior. The checker now scans 1,085 files cleanly, with 47 focused Kanban and 125 Browser Use tests passing.
- Fork workflow portability is explicit: upstream-only larger-runner labels remain selected in `NousResearch/hermes-agent`, while ordinary personal-fork lanes fall back to `ubuntu-latest`/`windows-latest`. The monolithic 38,000-plus-test Python lane remains on the upstream 96-core runner; forks use a focused portability/local-delta smoke lane plus E2E because forcing that suite onto four shared cores causes massive scheduler contention, per-test timeout noise and a 30-minute cancellation rather than useful validation. This prevents inaccessible-runner queues and resource-starvation false failures without reducing upstream coverage.
- The one CPU-sensitive SessionsPage routing regression uses an explicit 15-second timeout on standard fork runners; its behavior assertions and all production code remain unchanged.
- Privileged JS auto-fix apply/PR automation remains enabled only in the upstream repository where its trusted GitHub App and repository policy exist. Personal forks stop after the read-only patch-generation diagnostic and therefore require neither a write token nor PR-approval permission.
- The ui-tui layout-cache regression waits up to ten seconds for the unchanged compensation invariant inside a fifteen-second test budget rather than assuming a fixed 40 ms sleep, removing a standard-runner scheduling race without weakening the expected call/value assertions.
- All seven config files pass official schema v40 checks. Runtime verification reports `ok: true` across the canonical Hermes venv, two explicitly approved coder project venvs and the single governed Browser Use venv. Seven Profile state databases, both Kanban databases, Cron executions, DLOM state and the production IPO database all pass SQLite `quick_check`.
- The Node security warning is closed by a lock-only update of `sanitize-html` from vulnerable 2.17.6 to fixed 2.17.7. The live tree resolves only 2.17.7; root and Web production audits report zero vulnerabilities. A clean isolated `npm ci --ignore-scripts` followed by Web typecheck, tests, lint and production build passed with no parent-package or dependency-range change; candidate and rollback material were deleted.
- Formal Cron inventory is now 17 jobs: 16 enabled and one intentionally paused (`tender-intelligence`). All 17 sandbox-resolved scripts exist and pass syntax checks. Historical execution outcomes remain telemetry and do not gate current eligibility. Kanban is terminal-only at closeout (`ma-detail`: 345 done, 10 archived; `internal-journal`: 55 done, 51 archived).
- GitHub had two independent failures. Git HTTPS fetches were anonymous because no credential helper was configured and therefore hit HTTP 429 rate limits; `gh auth setup-git` restored the authenticated helper and a full fetch/prune then succeeded. Separately, `ci.yaml` failed before creating any job because its `detect-changes` composite action was passed two undefined checkout-only inputs; removing those inputs restores dispatch without changing classification behavior. The governed local branch is fast-forwarded to the fork and verified there; floating official `origin/main` remains next-cycle input and is not merged into the immutable production base `73f68362b3f639b97352a5dedc9e74b10520a84f`.
...
- Production runtime code is based on the owner-frozen upstream `73f68362b3f639b97352a5dedc9e74b10520a84f` on canonical branch `main`; subsequent local commits in this closeout are governance-documentation only. The exact live HEAD must equal `services/python-runtime/expected-source-commit` and is recorded in the external `current-state.md` after each documentation closeout. The editable distribution, imports, seven Gateway units and shared Python services all resolve through `/home/dionysos/.hermes/hermes-agent` and its single canonical venv. `uv --no-config pip check` reports all 262 packages compatible.
...
- All seven formal Profiles passed the official schema v40 migration/config checks, state-database `quick_check`, active Gateway/runtime checks and one real primary-model response. A semantic before/after comparison proves that v40 changed only `_config_version` and removed the three obsolete explicit `model_catalog.ttl_hours` overrides selected by the official migration; models, fallbacks, platforms, toolsets, MCP, TTS/STT, compression, Cron and Kanban configuration are unchanged. Usage receipts proved the configured route without fallback: default/coordinator/coder `openai-codex/gpt-5.6-sol`, crawler `minimax-cn/MiniMax-M3`, writer `zai/glm-5.3-flash`, supporter `deepseek/deepseek-v4-pro`, auditor1 `kimi-coding/k3`. Seven independent post-cutover Browser Use calls also returned `Example Domain` through each Profile's private run-owned Chrome path.
- The formal Cron inventory is now 17 jobs: 16 enabled and one intentionally paused (`tender-intelligence`). All 17 sandbox-resolved scripts exist and pass Python/shell syntax checks; all Profile Cron doctors pass; execution databases are healthy. The 2026-09-03 closeout readback found the scheduled `industry-intelligence` and serialized `ipo-dlom` executions legitimately running under the live crawler Gateway; both jobs' preceding recorded outcomes are `ok`. Historical failed executions remain telemetry, not launch gates. Cron writes remain Profile-local and atomic.
- The two formal Kanban boards (`ma-detail`, `internal-journal`) pass SQLite checks; every current card is terminal (`ma-detail`: 341 done, 10 archived; `internal-journal`: 55 done, 51 archived), with no claimable or running card. Non-success and one `scheduled` value in `task_runs` are immutable historical execution telemetry; the `scheduled` row's owning card is archived and is not a live queue item.
...
- Prior upgrade errors are closed: `uv --no-config` preserves `cryptography==50.0.0`; MinerU public/service/watchdog scripts all pass `bash -n`; Chrome resolves to the governed 152.0.7977.75 binary; writer `state.db` passes `quick_check`; CI-003 filters unreachable tags using a full graph; the final external cutover completed without a surviving rollback/candidate consumer.
...
- The central browser runtime contains exactly Chrome 152.0.7977.75, activation-deferred Lightpanda 0.4.0, and Browser Use 0.13.8 with Browser Harness 0.1.9; agent-browser remains frozen at Hermes' npm-native 0.26.0 contract. The upgrade-created Lightpanda 0.3.7 downgrade and service-layer agent-browser wrapper were removed after owner correction because they served no production requirement. Chrome exposed all 39/39 CDP methods used by Hermes; real built-in Chrome navigation/snapshot/cleanup and all seven managed `browser_exec` Profile paths passed after native-entry restoration. The 60-file Browser/CDP matrix passed 877 tests with one intentional OOPIF skip. Old browser candidates, processes, sockets, ports and profiles were removed.
...
- A shared Crawler browser contract governs the central Chrome entry, rejects historical fixed ports 9222-9228 and enforces the no-download environment. The obsolete shared-fixed-browser assertion was repaired to create its own unrelated sentinel, and the crawler browser suite passed 68/68. Five distinct Runner browser implementations started concurrently on OS-assigned ports, Tender's formal smoke passed, and every port/profile/process cleaned. A generic Kanban worker then exercised LP-017 through its formal entry: the final card completed with `KANBAN_RUN_OWNED_OK`, central Browser Use 0.13.7, random port 37139, PAC, all four no-download guards, no uvx and zero cache delta. A deliberately short Kanban task also reproduced upstream timeout orphaning before LP-018; its residue was stopped and removed, Chrome was rebound to the worker PGID, and timeout group termination gained a dedicated regression.
...
- Final regressions include Browser Use 90/90, browser wildcard 460 passed with 7 deselected, managed Browserbase/Modal 7/7, Kanban core/tool 60/60, config 76/76, policy Runner 30/30 and Crawler browser-contract 4/4. The out-of-scope redline delta was removed and is not part of the upgrade.
