...
- GitHub CLI API access was healthy, but Git HTTPS fetches were anonymous because no credential helper was configured and therefore hit GitHub HTTP 429 rate limits. `gh auth setup-git` restored the authenticated helper and a full fetch/prune then succeeded. The governed local branch is fast-forwarded to the fork and verified there; floating official `origin/main` remains next-cycle input and is not merged into the immutable production base `73f68362b3f639b97352a5dedc9e74b10520a84f`.
